...
func ServerAccountRequest(opts K8sRequestOption) (string, error) {
	...
	if tokenErr != nil {
		return "", &errors.RuntimeError{Err: tokenErr, CustomMsg: "load K8s service account token error."}
	}
	...
	if len(opts.Url) == 0 {
		...
		if urlErr != nil {
			return "", &errors.RuntimeError{Err: urlErr, CustomMsg: "err found while searching local K8s apiserver addr."}
		}
	}
	...
	if err != nil {
		return "", &errors.RuntimeError{Err: err, CustomMsg: "err found while generate post request in net.http ."}
	}
	...
	if err != nil {
		return "", &errors.RuntimeError{Err: err, CustomMsg: "err found in post request."}
	}
	...
	if !IntContains(MaybeSuccessfulStatuscodeList, resp.StatusCode) {
